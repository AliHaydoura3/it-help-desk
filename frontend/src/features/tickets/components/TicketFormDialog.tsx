...
import { TICKET_PRIORITIES, type Ticket, type TicketCategory } from "../types/ticket";
import { ticketSchema, type TicketFormData } from "../validation/ticketSchema";
import { useTicketCategories } from "../hooks/useTicketCategories";

const EMPTY_CATEGORIES: { category: TicketCategory; displayName: string }[] = [];
...
export function TicketFormDialog({ open, ticket, pending, onClose, onSubmit }: { open: boolean; ticket: Ticket | null; pending: boolean; onClose: () => void; onSubmit: (data: TicketFormData) => Promise<void> }) {
  ...
  const activeCategories = categoriesQuery.data ?? EMPTY_CATEGORIES;
  const categoryOptions = ticket && !activeCategories.some((item) => item.category === ticket.category)
    ? [{ category: ticket.category, displayName: `${ticket.category === "AccessRequest" ? "Access Request" : ticket.category} (inactive)` }, ...activeCategories]
    : activeCategories;
  ...
  useEffect(() => {
    if (open) reset(ticket ? { title: ticket.title, description: ticket.description, category: ticket.category, priority: ticket.priority } : { title: "", description: "", category: (activeCategories[0]?.category ?? "Hardware") as TicketCategory, priority: "Medium" });
  }, [open, reset, ticket, activeCategories]);
  if (!open) return null;
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="space-y-4 p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Category" error={categoriesQuery.isError ? "Ticket categories could not be loaded." : undefined}><select disabled={categoriesQuery.isLoading || categoryOptions.length === 0} className="h-9 w-full rounded-lg border bg-background px-3 text-sm disabled:opacity-50" {...register("category")}>{categoryOptions.map((item) => <option key={item.category} value={item.category}>{item.displayName}</option>)}</select></Field>
        ...
        <footer className="flex justify-end gap-2 border-t bg-muted/30 p-4"><Button disabled={pending} onClick={onClose} type="button" variant="outline">Cancel</Button><Button disabled={pending || categoriesQuery.isLoading || categoryOptions.length === 0} type="submit">{pending && <LoaderCircle className="animate-spin" />}{ticket ? "Save changes" : "Create ticket"}</Button></footer>
      </form>
    </section>
  </div>;
}
